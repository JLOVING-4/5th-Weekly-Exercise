...
```{r}
lettuce_graph2 <- garden_harvest %>% 
  filter(vegetable == "lettuce") %>% 
  group_by(variety) %>% 
  summarize(weight = n()) %>% 
  ggplot(aes(x = reorder(variety, weight),
             y = weight)) +
  geom_bar(stat = "identity") +
  coord_flip() +
  labs(title = "Number of Times a Variety of Lettuce was Harvested from Most to Least", 
       x = "", y= "")

ggplotly(lettuce_graph2)

```

```{r}
beets_graph2 <- garden_harvest %>% 
  ...
  ggplot(aes(x = date, 
             ...
             color = variety)) +
  geom_line() +
  labs(title = "Cumulative Weight of Beets Harvested Over Time",
       x = "", y ="")

ggplotly(beets_graph2)
```
...
```{r, eval=FALSE}
indv_departures_year17 <- small_trains %>% 
  filter(year == "2017")
indv_departures_year17
...
indv_departures_year17 %>%
  ...
  labs(title = "Number of Trips Departing from Paris Lyon Station in 2017",
       subtitle = "Arrival Station: {closest_state}",
  x = "",
  y = "")
anim_save("real_departures_q2.gif")
```
```{r, echo=FALSE}
knitr::include_graphics("real_departures_q2.gif")
```
...
garden_harvest %>% 
  ...
  labs(title = "Cumulative Tomato Harvest (lbs)",
       ...
       x = "",
       y = "") +
anim_save("real_tomatoes2.gif")  
  
```
```{r, echo=FALSE}
knitr::include_graphics("real_tomatoes2.gif")
```
...
anim_save("lisa's_bike_trip.gif")
```

```{r, echo=FALSE}
knitr::include_graphics("lisa's_bike_trip.gif")
```

  I like this type of map more and being able to actually how things changed over time with the actual movement of the bike. 
...
panama_combined <-bind_rows(panama_swim %>% 
                              mutate(event_image = "https://github.com/llendway/animation_and_interactivity/master/swimmer.jpg"),
                            panama_bike %>% 
                              mutate(event_image = "https://raw.githubusercontent.com/llendway/animation_and_interactivity/master/bike.png"),
                            panama_run %>% 
                              mutate(event_image = "https://github.com/llendway/animation_and_interactivity/master/runner.png"))
panama_combined
...
ggmap(panama_map) +
  ...
  labs(title = "Heather's PanAm Triathlon",
       subtitle = "Time: {frame_along}")
anim_save("heather's_triad.gif")
```

```{r, echo=FALSE}
knitr::include_graphics("heather's_triad.gif")
```
...
covid19 %>% 
  ...
  scale_x_log10(breaks = scales::trans_breaks("log10",
                                              function(x) 10^x), 
                labels = scales::comma) +
  labs(title = "Reproduction of 'Trajectory of US Covid Cases'",
...
```
  ...
  The graph shows how in the beginning almost every state was all increasing at the same rate, and then there's separation around the time when states started to enact lockdowns. The growth seems a a bit stagnant throughout the latter half of 2020, but still steadily climbs. 
...
animate(animated_country_covid,
        nframes = 200)
anim_save("country_covid_map.gif")  
```
```{r, echo=FALSE}
knitr::include_graphics("country_covid_map.gif")
```

It seems when the data is organized this way, smaller states with a smaller population size, for example North Dakota, confirm more cases faster than larger states such as California
